...
public class SavedList<E extends Serializable> extends AbstractList<E> implements Reloadable {
    ...
    public SavedList(File fileInst) {
        if (fileInst.exists()) {
			...
			fileInst = new File(fileInst.toString());
		}
    } // construct
    ...
    private void save() {
		try {
			FileOutputStream fileInstFos = new FileOutputStream(fileInst);
			ObjectOutputStream objOs = new ObjectOutputStream(fileInstFos);
